  ...
  if (bSuccess == false) 
  {
   ...
   break;
	...
	int width = image.cols;
 	...
 	previousValues.resize(height * width / 100);
 	
	for(int pixel_Y = 0; pixel_Y <= (height - 10); pixel_Y += 10)
	{
		for(int pixel_X = 0; pixel_X <= (width - 10); pixel_X += 10)
		{
		...
		brightness = 1.7*brightness;
			
		Mat roi = image (Rect(pixel_X, pixel_Y, 10, 10));
			...
			{
				currentBrightness = (brightness/25)-1;
			}

			int valueLocation = (pixel_Y / 10) * (width / 10) + (pixel_X / 10);
			int lastBrightnessValue = previousValues [valueLocation];
			
			if (currentBrightness == lastBrightnessValue)
			{
				blocks [currentBrightness].copyTo(roi);
			}
			else if (currentBrightness > lastBrightnessValue)
			{
				lastBrightnessValue = currentBrightness;
				blocks [lastBrightnessValue].copyTo(roi);
			}
			else
			{
				--lastBrightnessValue;
			...
			previousValues [valueLocation] = lastBrightnessValue;
